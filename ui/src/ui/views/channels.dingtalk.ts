...
export function renderDingtalkCard(params: {
  ...
  const { props, dingtalk, dingtalkAccounts, accountCountLabel } = params;
  ...
  const configured = typeof dingtalk?.configured === "boolean" ? dingtalk.configured : false;
  const running = typeof dingtalk?.running === "boolean" ? dingtalk.running : false;
  const lastStartAt = typeof dingtalk?.lastStartAt === "number" ? dingtalk.lastStartAt : null;
  const lastProbeAt = typeof dingtalk?.lastProbeAt === "number" ? dingtalk.lastProbeAt : null;
  const lastError = typeof dingtalk?.lastError === "string" ? dingtalk.lastError : null;
  const probe = dingtalk?.probe as { ok?: boolean; status?: string; error?: string } | undefined;

  return html`
    <div class="card">
      <div class="card-title">钉钉</div>
